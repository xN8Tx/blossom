...
import type { UserProfileDB, UsersDB } from '../typings';

type UserWithoutPassword = Omit<UsersDB, 'password'>;

class UsersAPI {
  async getById(id: number): Promise<UserProfileDB | null> {
    try {
      ...
      logger.error('⚠️ Error in UsersAPI getById', (error as Error)?.message);
      return null;
    }
  }
  async getAllById(id: number): Promise<UserWithoutPassword | null> {
    try {
  ...
  async post(
    ...
    try {
      ...
      logger.error('⚠️ Error in UsersAPI post', (error as Error)?.message);
      return null;
    }
  }
  async edit(
    ...
    try {
      ...
      logger.error('⚠️ Error in UsersAPI edit', (error as Error)?.message);
      return null;
    }
  }
  async editEmail(
    ...
    try {
      ...
      logger.error('⚠️ Error in UsersAPI editEmail', (error as Error)?.message);
      return null;
    }
  }
  ...
  async setAvatar(
    ...
    try {
      ...
      logger.error('⚠️ Error in UsersAPI setAvatar', (error as Error)?.message);
      return null;
    }
  }
}
