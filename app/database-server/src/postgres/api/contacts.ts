...
import type { ContactDB, ContactUserDB } from '../typings';
...
class ContactsAPI {
  async get(id: number): Promise<ContactUserDB[] | null> {
    try {
      ...
      logger.error('⚠️ Error in ContactsAPI get', (error as Error)?.message);
      return null;
    }
  }
  async getById(
  ...
  async post(userId: number, contactId: number): Promise<ContactUserDB | null> {
    try {
      ...
      logger.error('⚠️ Error in ContactsAPI post', (error as Error)?.message);
      return null;
    }
  }
  async delete(userId: number, contactId: number): Promise<ContactDB | null> {
    try {
      ...
      logger.error('⚠️ Error in ContactsAPI delete', (error as Error)?.message);
      return null;
    }
  }
}
